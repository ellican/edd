...
function getCsrfToken() {
    ...
    console.warn('[ACCOUNT] CSRF token not found - requests may fail');
    return '';
}
    ...
    fetch('/api/account/get-profile.php')
        ...
        .then(data => {
            if (data.success && data.data) {
                const fields = ['first_name', 'last_name', 'email', 'phone', 'gender', 'date_of_birth'];
                ...
                console.warn('[ACCOUNT] Profile load failed:', data.message || 'Unknown error');
            }
        })
        .catch(err => {
...
async function handleProfileUpdate(e) {
    ...
    if (!e.target) {
        console.error('[ACCOUNT] handleProfileUpdate: no target form');
        return;
    }
    ...
    try {
        ...
        if (result.success) {
            ...
            console.warn('[ACCOUNT] Profile update failed:', result.error);
...
// Export functions to window for onclick handlers (backwards compatibility)
// These are used by inline onclick attributes in account.php
