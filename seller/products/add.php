...
// Check if seller has KYC verification approved
if (function_exists('canSellerListProducts')) {
    $kycStatus = canSellerListProducts(Session::getUserId());
    if (!$kycStatus['can_sell']) {
        $page_title = 'KYC Verification Required';
        includeHeader($page_title);
        ?>
        <div class="container my-4">
            <div class="alert alert-warning">
                <h4><i class="fas fa-exclamation-triangle"></i> KYC Verification Required</h4>
                <p><?php echo htmlspecialchars($kycStatus['message']); ?></p>
                <?php if ($kycStatus['kyc_status'] === 'not_submitted'): ?>
                    <p class="mt-3">To list products on our platform, you need to complete KYC (Know Your Customer) verification. This helps us maintain a trusted marketplace.</p>
                    <a href="/seller/kyc.php" class="btn btn-primary">Complete KYC Verification</a>
                <?php elseif ($kycStatus['kyc_status'] === 'pending' || $kycStatus['kyc_status'] === 'under_review'): ?>
                    <p class="mt-3">Your KYC verification is currently under review. You'll be able to list products once your verification is approved.</p>
                    <a href="/seller/dashboard.php" class="btn btn-secondary">Back to Dashboard</a>
                <?php elseif ($kycStatus['kyc_status'] === 'rejected'): ?>
                    <p class="mt-3">Your KYC verification was rejected. Please contact support or resubmit your documents.</p>
                    <a href="/seller/kyc.php" class="btn btn-primary">Resubmit KYC Documents</a>
...
tinymce.init({
    ...
    toolbar: 'undo redo | formatselect | bold italic underline strikethrough | alignleft aligncenter alignright alignjustify | bullist numlist outdent indent | link image | table | removeformat | code | help',
    branding: false,
    promotion: false,
