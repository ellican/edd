...
$productsQuery = "
    SELECT p.*, c.name as category_name,
           ...
           CASE WHEN sp.id IS NOT NULL AND sp.status = 'active' AND sp.sponsored_until > NOW() THEN 1 ELSE 0 END as is_sponsored
    FROM products p
        ...
        <?php if (!empty($products)): ?>
            <div class="products-header">
                <div class="bulk-actions">
                    ...
                    <div class="bulk-actions-dropdown" style="display: none;" id="bulkActionsDropdown">
                        <button onclick="bulkActivate()" class="btn btn-sm btn-success">Activate</button>
                        <button onclick="bulkDeactivate()" class="btn btn-sm btn-warning">Deactivate</button>
                ...
                <?php foreach ($products as $product): ?>
                    <div class="product-card" data-product-id="<?php echo $product['id']; ?>">
                        <div class="product-checkbox">
                            <input type="checkbox" class="product-select" value="<?php echo $product['id']; ?>" onchange="updateBulkActions()">
                        </div>
                        <div class="product-image">
                            ...
                            <?php if ($product['image_count'] > 1): ?>
                                <div class="image-count-badge"><?php echo $product['image_count']; ?> 📷</div>
                            <?php endif; ?>
                        </div>
                        </div>
                        ...
                        <div class="product-info">
                            <div class="product-header">
                                ...
                                <div class="product-actions-menu">
                                    ...
                                    <div class="actions-dropdown">
                                        ...
                                        <a href="/product.php?id=<?php echo $product['id']; ?>" target="_blank">👁️ View</a>
                                        <button onclick="duplicateProduct(<?php echo $product['id']; ?>)">📋 Duplicate</button>
                                        <hr class="dropdown-divider">
                                        <button onclick="toggleFeature(<?php echo $product['id']; ?>, <?php echo $product['is_featured'] ? 'false' : 'true'; ?>)">
                                            <?php echo $product['is_featured'] ? '⭐ Unfeature' : '⭐ Feature'; ?>
                                        </button>
                                        <button onclick="showSponsorModal(<?php echo $product['id']; ?>)">
                                            🚀 Sponsor Product
                                        </button>
                                        <hr class="dropdown-divider">
                            ...
                            <div class="status-toggle-inline">
                                <label class="toggle-switch">
                                    <input type="checkbox" 
                                           ...
                                           onchange="toggleProductStatusInline(<?php echo $product['id']; ?>, this.checked)">
                                    <span class="toggle-slider"></span>
                                </label>
                                <span class="toggle-label"><?php echo $product['status'] === 'active' ? 'Active' : 'Inactive'; ?></span>
                            </div>
                            ...
                            <div class="product-stats">
                                <div class="stat-group">
                                    <div class="stat <?php echo ($product['stock_quantity'] <= $product['min_stock_level']) ? 'stat-warning' : ''; ?>">
                                    ...
                                    <?php if ($product['purchase_count'] > 0 && $product['view_count'] > 0): ?>
                                    <div class="stat">
                                        <span class="stat-value"><?php echo round(($product['purchase_count'] / $product['view_count']) * 100, 1); ?>%</span>
...
// Inline status toggle
function toggleProductStatusInline(productId, isActive) {
    ...
    .then(data => {
        if (data.success) {
            ...
            showToast(data.message || 'Product status updated', 'success');
        } else {
            alert('Error updating product status: ' + (data.message || 'Unknown error'));
            // Revert checkbox
...
function toggleFeature(productId, shouldFeature) {
    fetch('/api/seller/products/feature.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-Token': '<?php echo csrfToken(); ?>'
...
// Show sponsor modal
function showSponsorModal(productId) {
    window.location.href = '/seller/ads/create.php?product_id=' + productId;
}
...
function bulkActivate() {
    ...
    if (confirm(`Activate ${productIds.length} product(s)?`)) {
        bulkUpdateStatus(productIds, 'active');
    }
}
...
function bulkDeactivate() {
    ...
    if (confirm(`Deactivate ${productIds.length} product(s)?`)) {
        bulkUpdateStatus(productIds, 'inactive');
    }
}
...
function bulkDelete() {
    ...
    if (confirm(`Delete ${productIds.length} product(s)? This action cannot be undone.`)) {
        bulkUpdateStatus(productIds, 'delete');
    }
}

function bulkUpdateStatus(productIds, action) {
    fetch('/api/seller/products/bulk-action.php', {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
            'X-CSRF-Token': '<?php echo csrfToken(); ?>'
