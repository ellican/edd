                ...
                case 'bulk_import':
                    ...
                    try {
                        while (($row = fgetcsv($handle)) !== false) {
                            ...
                            // Insert user
                            $stmt = $pdo->prepare("
                                INSERT INTO users (username, email, pass_hash, role, first_name, last_name, status, created_at)
                                VALUES (?, ?, ?, ?, ?, ?, ?, NOW())
                            ");
...
<head>
    ...
    <style>
        ...
        /* Better button styling for pending users */
...
<body>
    ...
    <div class="container">
        ...
        <div class="card mb-3">
            <div class="card-body">
                <div class="row align-items-end">
                    <div class="col-md-4">
                        <label for="user-search" class="form-label"><i class="fas fa-search me-2"></i>Search Users</label>
                        <input type="text" id="user-search" class="form-control" placeholder="Search by name, email, or username...">
                    </div>
                    <div class="col-md-2">
                    ...
                    <div class="col-md-2">
                        ...
                        <div class="btn-group w-100" role="group">
                            <button type="button" class="btn btn-outline-primary active" id="view-cards" onclick="setViewMode('cards')">
                                <i class="fas fa-th"></i>
                            </button>
                            <button type="button" class="btn btn-outline-primary" id="view-list" onclick="setViewMode('list')">
        ...
        <!-- Enhanced Users Grid -->
        <div id="users-container-cards" class="row">
            <?php if (empty($users)): ?>
                <div class="col-12">
            ...
            <?php else: ?>
                <?php foreach ($users as $user): ?>
                    <div class="col-lg-6 col-xl-4 user-item" 
                         data-username="<?php echo htmlspecialchars($user['username'] ?? ''); ?>"
                         data-email="<?php echo htmlspecialchars($user['email'] ?? ''); ?>"
                         data-name="<?php echo htmlspecialchars(($user['first_name'] ?? '') . ' ' . ($user['last_name'] ?? '')); ?>"
                         data-role="<?php echo htmlspecialchars($user['role'] ?? 'customer'); ?>"
        ...
        <div id="users-container-list" class="card" style="display: none;">
                            ...
                            <?php if (!empty($users)): ?>
                                <?php foreach ($users as $user): ?>
                                    <tr class="user-item-list"
                                        ...
                                        data-status="<?php echo htmlspecialchars($user['status'] ?? 'active'); ?>"
                                        data-created="<?php echo htmlspecialchars($user['created_at'] ?? ''); ?>">
                                        <td><input type="checkbox" class="user-select" value="<?php echo $user['id']; ?>"></td>
                                        <td>
                                            <div class="d-flex align-items-center">
                                                ...
                                                <div>
                                                    <strong><?php echo htmlspecialchars(($user['first_name'] ?? 'Unknown') . ' ' . ($user['last_name'] ?? 'User')); ?></strong>
                                                    <div class="text-muted small">@<?php echo htmlspecialchars($user['username'] ?? 'unknown'); ?></div>
                                                </div>
                                            </div>
                                        </td>
                                        ...
                                        <td>
                                            <span class="badge bg-<?php 
                                                echo ($user['role'] ?? 'customer') === 'admin' ? 'danger' : 
                                                    (($user['role'] ?? 'customer') === 'seller' ? 'primary' : 'secondary'); 
                                            ?>">
                                                <?php echo ucfirst($user['role'] ?? 'customer'); ?>
                                            </span>
                                        </td>
                                        <td>
                                            <span class="badge bg-<?php 
                                                echo ($user['status'] ?? 'active') === 'active' ? 'success' : 
                                                    (($user['status'] ?? 'active') === 'pending' ? 'warning' : 'danger'); 
                                            ?>">
                                                <?php echo ucfirst($user['status'] ?? 'active'); ?>
                                            </span>
                                        </td>
                                        ...
                                        <td>
                                            <div class="btn-group btn-group-sm">
                                                <a href="?action=edit&id=<?php echo $user['id']; ?>" class="btn btn-outline-primary" title="Edit">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <a href="?action=view&id=<?php echo $user['id']; ?>" class="btn btn-outline-info" title="View">
                                                    <i class="fas fa-eye"></i>
                                                </a>
                                                <?php if (($user['status'] ?? 'active') === 'active'): ?>
                                                    <button class="btn btn-outline-warning" onclick="suspendUser(<?php echo $user['id']; ?>)" title="Suspend">
                                                        <i class="fas fa-pause"></i>
                                                    </button>
                                                <?php else: ?>
                                                    <button class="btn btn-outline-success" onclick="activateUser(<?php echo $user['id']; ?>)" title="Activate">
    ...
    async function suspendUser(userId) {
        ...
        try {
            const response = await fetch('/api/admin/users/deactivate.php', {
                method: 'POST',
                headers: {
            ...
            if (data.success) {
                showNotification(data.message || 'User suspended successfully', 'success');
    ...
    async function activateUser(userId) {
        ...
        try {
            const response = await fetch('/api/admin/users/activate.php', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-Csrf-Token': '<?= generateCSRFToken() ?>'
            ...
            if (data.success) {
                showNotification(data.message || 'User activated successfully', 'success');
                setTimeout(() => location.reload(), 1500);
            } else {
                showNotification(data.error || 'Failed to activate user', 'error');
            }
        } catch (error) {
            console.error('Error:', error);
            showNotification('Network error: ' + error.message, 'error');
    ...
    function setViewMode(mode) {
        const cardsContainer = document.getElementById('users-container-cards');
        const listContainer = document.getElementById('users-container-list');
        ...
        if (mode === 'cards') {
            ...
            localStorage.setItem('userViewMode', 'cards');
        } else {
    ...
    function filterUsers() {
        ...
        // Get all user items (both views)
        const userCards = Array.from(document.querySelectorAll('.user-item'));
        const userListRows = Array.from(document.querySelectorAll('.user-item-list'));
        ...
        // Sort items
        sortUsers(sortOrder, userCards, userListRows);
    }
    
    function sortUsers(order, userCards, userListRows) {
        const cardsContainer = document.getElementById('users-container-cards');
        ...
        // Sort cards
        userCards.sort(sortFn).forEach(card => {
            cardsContainer.appendChild(card);
        });
        
        // Sort list rows
        userListRows.sort(sortFn).forEach(row => {
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const savedViewMode = localStorage.getItem('userViewMode') || 'cards';
        ...
        document.getElementById('sort-order').addEventListener('change', filterUsers);
