...
if (isset($_FILES['attachment']) && $_FILES['attachment']['error'] === UPLOAD_ERR_OK) {
    ...
    if (in_array($file_ext, $allowed_types) && $file['size'] <= 5 * 1024 * 1024) {
        ...
        if (move_uploaded_file($file['tmp_name'], $file_path)) {
            $attachment_path = 'uploads/email_attachments/' . $filename;
        }
    }
}
    ...
    try {
        ...
        foreach ($recipients as $recipient) {
            ...
            if (in_array('email', $send_via)) {
                // Store attachment path in metadata JSON
                $metadata = $attachment_path ? json_encode(['attachment' => $attachment_path]) : null;
                
                $stmt = $pdo->prepare("
                    INSERT INTO email_queue (recipient_email, recipient_name, subject, body, status, created_at, metadata)
                    VALUES (?, ?, ?, ?, 'pending', NOW(), ?)
                ");
                $stmt->execute([
                    $recipient['email'],
        ...
        <div class="compose-container">
            <form method="POST" enctype="multipart/form-data">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="message" id="message_editor" class="form-control" rows="10" placeholder="Enter your message..." required></textarea>
                    <small class="text-muted">Use the rich text editor to format your message professionally</small>
                </div>
                
                <!-- File Attachment -->
                <div class="mb-4">
                    <label class="form-label"><strong>Attachment (Optional)</strong></label>
                    <input type="file" name="attachment" class="form-control" accept=".pdf,.jpg,.jpeg,.png,.gif,.doc,.docx,.xls,.xlsx,.txt">
                    <small class="text-muted">Max file size: 5MB. Supported: PDF, Images, Documents</small>
                </div>
