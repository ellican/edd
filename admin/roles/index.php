...
// Get data from database with enhanced error handling
$pdo = null;
$roles = [];
$permissions = [];
$permissionsByModule = [];
...
try {
    ...
    try {
        ...
        error_log("Admin Roles - Database connection failed: " . $dbConnError->getMessage());
        error_log("Connection details - Host: " . (defined('DB_HOST') ? DB_HOST : 'undefined') . 
                  ", Database: " . (defined('DB_NAME') ? DB_NAME : 'undefined'));
        
        // Show detailed error in development, generic in production
        if ($isDevelopment) {
            $database_error = 'Database connection error: ' . $dbConnError->getMessage() . 
                            ' (Host: ' . (defined('DB_HOST') ? DB_HOST : 'undefined') . 
                            ', Database: ' . (defined('DB_NAME') ? DB_NAME : 'undefined') . ')';
        } else {
            $database_error = 'Error loading data from database. Please check database connection.';
        }
        throw $dbConnError;
    }
    
    // Attempt to fetch roles
    try {
        $stmt = $pdo->query("SELECT * FROM roles ORDER BY level DESC, name");
        $roles = $stmt->fetchAll(PDO::FETCH_ASSOC);
    } catch (Exception $queryError) {
        error_log("Admin Roles - Failed to query roles table: " . $queryError->getMessage());
        ...
        if ($isDevelopment) {
            $database_error = 'Query error (roles): ' . $queryError->getMessage();
        } else {
            $database_error = 'Error loading data from database. The roles table may not exist.';
        }
        throw $queryError;
    }
    
    // Attempt to fetch permissions
    try {
        $stmt = $pdo->query("SELECT * FROM permissions ORDER BY module, name");
        $permissions = $stmt->fetchAll(PDO::FETCH_ASSOC);
    } catch (Exception $queryError) {
        error_log("Admin Roles - Failed to query permissions table: " . $queryError->getMessage());
        ...
        if ($isDevelopment) {
            $database_error = 'Query error (permissions): ' . $queryError->getMessage();
        } else {
            $database_error = 'Error loading data from database. The permissions table may not exist.';
        }
        throw $queryError;
    }
    ...
    // Check if we got empty results vs an error - this is NOT an error condition
    if (empty($roles) && empty($permissions)) {
        // This is logged but not shown as an error to the user
        error_log("Admin Roles - No roles or permissions found in database. Database may need seeding.");
    }
    
} catch (Exception $e) {
    // Error has already been logged above, just ensure we have an error message set
    if (!$database_error) {
        error_log("Admin Roles - Unexpected error: " . $e->getMessage());
        $database_error = 'Error loading data from database.';
    }
    $_SESSION['error_message'] = $database_error;
}

// Get current role for editing
$currentRole = null;
$currentRolePermissions = [];
if ($action === 'edit' && $role_id && $pdo) {
    try {
        ...
        if ($currentRole) {
            try {
                ...
                error_log("Admin Roles - Failed to fetch role permissions: " . $permError->getMessage());
                // Don't fail completely, just show empty permissions
                $currentRolePermissions = [];
            }
        }
    } catch (Exception $e) {
        error_log("Admin Roles - Failed to fetch role details for editing: " . $e->getMessage());
        $_SESSION['error_message'] = 'Error loading role details. The role may not exist.';
    }
}
                            ...
                            <?php else: ?>
                                ...
                                if ($pdo) {
                                    try {
                                        ...
                                        error_log("Admin Roles - Failed to count users for role {$role['id']}: " . $e->getMessage());
                                        $userCount = 0;
                                    }
                                }
                                ?>
