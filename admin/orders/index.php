...
if ($action === 'view' && $order_id) {
    try {
        ...
        // Calculate totals from order items if they're missing/zero in the order
        if ($currentOrder && ($currentOrder['subtotal_amount'] == 0 || $currentOrder['total_amount'] == 0)) {
            $calculatedSubtotal = 0;
            foreach ($orderItems as $item) {
                $calculatedSubtotal += ($item['price'] ?? 0) * ($item['quantity'] ?? 0);
            }
            
            // Use calculated values if database values are zero
            if ($currentOrder['subtotal_amount'] == 0) {
                $currentOrder['subtotal_amount'] = $calculatedSubtotal;
            }
            if ($currentOrder['total_amount'] == 0) {
                // Calculate total: subtotal + tax + shipping - discount
                $currentOrder['total_amount'] = $calculatedSubtotal 
...
try {
    ...
    // Calculate totals from order items if they're missing/zero
    foreach ($orders as $key => $order) {
        if (($order['subtotal_amount'] == 0 || $order['total_amount'] == 0) && $order['id']) {
            try {
                ...
                // Update the order array with calculated values
                if ($order['subtotal_amount'] == 0) {
                    $orders[$key]['subtotal_amount'] = $calculatedSubtotal;
                }
                if ($order['total_amount'] == 0) {
                    // Calculate total: subtotal + tax + shipping - discount
                    $orders[$key]['total_amount'] = $calculatedSubtotal 
                        + ($order['tax_amount'] ?? 0) 
                        + ($order['shipping_amount'] ?? 0)
                        - ($order['discount_amount'] ?? 0);
                }
            } catch (Exception $calcEx) {
                error_log("Error calculating order totals for order {$order['id']}: " . $calcEx->getMessage());
            }
        }
    }
...
<head>
    ...
    <style>
        ...
        /* Enhanced button colors for different actions */
                        ...
                        <table class="table table-hover mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <strong class="text-primary">#<?php echo $orderData['id']; ?></strong>
                                    </td>
                                    <td>
                                        <div>
                                    ...
                                    <td>
                                        <div><?php echo date('M d, Y', strtotime($orderData['created_at'])); ?></div>
                                        <small class="text-muted"><?php echo date('H:i', strtotime($orderData['created_at'])); ?></small>
                                    </td>
                                    <td>
                                        <strong class="text-secondary">$<?php echo number_format($orderData['subtotal_amount'] ?? 0, 2); ?></strong>
                                    </td>
                                    <td>
                                        <strong class="text-success">$<?php echo number_format($orderData['total_amount'] ?? 0, 2); ?></strong>
                                    </td>
                                    <td>
                                        <?php
                                        $statusConfig = [
                                            'pending' => ['class' => 'warning', 'icon' => 'clock', 'label' => 'Pending'],
                                            'processing' => ['class' => 'info', 'icon' => 'cog', 'label' => 'Processing'],
                                            'shipped' => ['class' => 'primary', 'icon' => 'truck', 'label' => 'Shipped'],
                                            'completed' => ['class' => 'success', 'icon' => 'check-circle', 'label' => 'Completed'],
                                            'cancelled' => ['class' => 'danger', 'icon' => 'times-circle', 'label' => 'Cancelled'],
                                            'refunded' => ['class' => 'secondary', 'icon' => 'undo', 'label' => 'Refunded']
                                        ];
                                        $status = $orderData['status'] ?? 'pending';
                                        $config = $statusConfig[$status] ?? ['class' => 'secondary', 'icon' => 'question', 'label' => ucfirst($status)];
                                        ?>
                                        <span class="badge bg-<?php echo $config['class']; ?> rounded-pill px-3 py-2">
                                    ...
                                    <td>
                                        <?php
                                        $paymentConfig = [
                                            'paid' => ['class' => 'success', 'icon' => 'check', 'label' => 'Paid'],
                                            'pending' => ['class' => 'warning', 'icon' => 'clock', 'label' => 'Pending'],
                                            'failed' => ['class' => 'danger', 'icon' => 'times', 'label' => 'Failed'],
                                            'refunded' => ['class' => 'info', 'icon' => 'undo', 'label' => 'Refunded']
                                        ];
                                        $payment = $orderData['payment_status'] ?? 'pending';
                                        $pConfig = $paymentConfig[$payment] ?? ['class' => 'secondary', 'icon' => 'question', 'label' => ucfirst($payment)];
                                        ?>
                                        <span class="badge bg-<?php echo $pConfig['class']; ?> rounded-pill px-3 py-2">
                                            <i class="fas fa-<?php echo $pConfig['icon']; ?> me-1"></i><?php echo $pConfig['label']; ?>
                                        </span>
                                    </td>
                                    <td>
                                        <div class="btn-group btn-group-sm" role="group">
                                            <a href="?action=view&id=<?php echo $orderData['id']; ?>" 
                                               class="btn btn-outline-info rounded-start" title="View Details">
                                            ...
                                            <button type="button" class="btn btn-outline-success rounded-end" 
                                                    title="Add Tracking" 
                                            ...
                                            <button type="button" class="btn btn-outline-danger rounded-end" 
                                                    title="Cancel Order" 
                                                    onclick="if(confirm('Are you sure you want to cancel this order?')) updateStatus(<?php echo $orderData['id']; ?>, 'cancelled')">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (statusForm) {
                statusForm.addEventListener('submit', function(e) {
                    ...
                    setTimeout(() => {
                        showNotification('Processing order status update...', 'info');
                    }, 100);
                });
            }
            ...
            if (trackingForm) {
                trackingForm.addEventListener('submit', function(e) {
                    ...
                    setTimeout(() => {
                        showNotification('Adding tracking information...', 'info');
                    }, 100);
                });
            }
