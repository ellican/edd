    ...
    try {
        switch ($_POST['action']) {
            ...
            case 'create_backup':
                ...
                // Create backup record using correct column names from schema
                $stmt = $pdo->prepare("
                    INSERT INTO backups (filename, file_path, backup_type, status, file_size, created_by, created_at) 
                    VALUES (?, ?, ?, 'in_progress', 0, ?, NOW())
                ");
                $stmt->execute([$filename, $file_path, $backup_type, $_SESSION['user_id']]);
            ...
            case 'retry_failed_jobs':
                ...
                // Try to use 'attempts' column first, fallback to 'retry_count' if not available
                $stmt = $pdo->prepare("
                    UPDATE jobs 
                    SET status = 'pending', 
...
try {
    ...
    // Get failed jobs
    // Use retry_count and max_retries (actual columns) and create aliases for backwards compatibility
    $stmt = $pdo->query("
        SELECT j.*,
            ...
            <div class="col-md-3 mb-3">
                <div class="stats-card">
                    <div class="stats-value text-primary">
                        <?php echo number_format((int)($stats['total_jobs'] ?? 0)); ?>
                    </div>
            ...
            <div class="col-md-3 mb-3">
                <div class="stats-card danger">
                    <div class="stats-value text-danger">
                        <?php echo number_format((int)($stats['failed_jobs'] ?? 0)); ?>
                    </div>
            ...
            <div class="col-md-3 mb-3">
                <div class="stats-card success">
                    <div class="stats-value text-success">
                        <?php echo number_format((int)($stats['total_backups'] ?? 0)); ?>
                    </div>
                    ...
                    <div class="card-body">
                        ...
                        <form method="POST" class="d-inline">
                            ...
                            <button type="submit" class="btn btn-warning">
                                Retry Jobs (<?php echo number_format((int)($stats['failed_jobs'] ?? 0)); ?>)
                            </button>
                        </form>
                    </div>
                </div>
                        ...
                        <div class="backup-item">
                            <div class="d-flex justify-content-between align-items-start">
                                <div>
                                    ...
                                    <br><small class="text-muted"><?php echo htmlspecialchars($backup['backup_type'] ?? ''); ?> backup</small>
                                ...
                                <div class="text-end">
                                    ...
                                    <br><a href="<?php echo htmlspecialchars($backup['file_path']); ?>" class="btn btn-sm btn-outline-primary mt-1">
