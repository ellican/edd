...
/**
 * Check if a user has KYC verified status
 * 
 * @param int $userId User ID to check
 * @return bool True if KYC is verified, false otherwise
 */
if (!function_exists('isKycVerified')) {
    function isKycVerified($userId) {
        try {
            ...
            error_log("Error checking KYC status for user {$userId}: " . $e->getMessage());
            return false;
        }
    }
}
...
/**
 * Check if a seller can list products (KYC must be verified)
 * 
 * @param int $userId User ID to check
 * @return array ['can_sell' => bool, 'message' => string, 'kyc_status' => string]
 */
if (!function_exists('canSellerListProducts')) {
    function canSellerListProducts($userId) {
        try {
            $pdo = db();
            
            // Check if user is a vendor/seller
            $stmt = $pdo->prepare("SELECT id, status FROM vendors WHERE user_id = ? LIMIT 1");
            $stmt->execute([$userId]);
            $vendor = $stmt->fetch();
            ...
            if (!$vendor) {
                return [
                    'can_sell' => false,
                    'message' => 'You must register as a seller first.',
                    'kyc_status' => 'not_registered'
                ];
            }
            
            // Check vendor account status
            if (isset($vendor['status']) && $vendor['status'] !== 'approved' && $vendor['status'] !== 'active') {
                return [
                    'can_sell' => false,
                    'message' => 'Your seller account is pending approval.',
                    'kyc_status' => 'vendor_pending'
                ];
            }
            ...
            if (!$kycVerification) {
                return [
                    'can_sell' => false,
                    'message' => 'You must complete KYC verification before you can sell. Please submit your KYC documents.',
            ...
            return [
                'can_sell' => false,
                'message' => 'Unable to verify your KYC status. Please contact support.',
                'kyc_status' => 'unknown'
            ];
            
        } catch (Exception $e) {
            error_log("Error checking seller eligibility for user {$userId}: " . $e->getMessage());
            return [
                'can_sell' => false,
                'message' => 'Unable to verify your seller status at this time. Please try again later.',
                'kyc_status' => 'error'
            ];
        }
    }
