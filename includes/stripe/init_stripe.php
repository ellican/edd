 ...
 * Uses user's session currency if available, otherwise falls back to env default
 * 
 * @return string Currency code in lowercase (e.g., 'usd', 'eur', 'rwf')
 */
function getStripeCurrency(): string {
    // Try to get currency from session (set by Currency class)
    if (class_exists('Session') && Session::get('currency_code')) {
        return strtolower(Session::get('currency_code'));
    }
