...
class StreamViewer extends BaseModel {
    ...
    public function addViewer($streamId, $userId = null, $sessionId = null, $ipAddress = null, $userAgent = null) {
        $stmt = $this->db->prepare("
            INSERT INTO {$this->table} 
            (stream_id, user_id, session_id, ip_address, user_agent, joined_at, is_active, is_fake)
    ...
    public function getActiveViewers($streamId, $includeFake = true) {
        // Get all active viewers (both real and fake combined)
        $sql = "
            SELECT sv.*, u.username, u.email
            FROM {$this->table} sv
            LEFT JOIN users u ON sv.user_id = u.id
            WHERE sv.stream_id = ? AND left_at IS NULL
        ";
        
        if (!$includeFake) {
            $sql .= " AND sv.is_fake = 0";
        }
        
        $sql .= " ORDER BY sv.joined_at ASC";
    ...
    public function getViewerStats($streamId) {
        $sql = "
            SELECT 
                COUNT(*) as total_viewers,
                SUM(CASE WHEN is_fake = 0 THEN 1 ELSE 0 END) as real_viewers,
                SUM(CASE WHEN is_fake = 1 THEN 1 ELSE 0 END) as fake_viewers
            FROM {$this->table}
