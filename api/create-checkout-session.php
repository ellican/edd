...
try {
    ...
    // Get currency instance for conversion
    $currency = Currency::getInstance();
    $userCurrency = $currency->getCurrentCurrency();
    
    // Calculate totals server-side (NEVER trust client-supplied amounts)
    // Prices in DB are in USD, convert to user's currency
    $subtotal = 0;
    foreach ($cartItems as $item) {
        $priceUSD = $item['price'] ?? 0;
        $priceInUserCurrency = $currency->convertPrice($priceUSD, $userCurrency);
        $subtotal += $priceInUserCurrency * ($item['quantity'] ?? 1);
    }
    
    // Calculate tax and shipping (in user's currency)
    $taxRate = 0.08; // 8% tax rate
    $freeShippingThresholdUSD = 50;
    $shippingCostUSD = 5.99;
    
    // Convert thresholds to user's currency
    $freeShippingThreshold = $currency->convertPrice($freeShippingThresholdUSD, $userCurrency);
    $shippingCost = $subtotal >= $freeShippingThreshold ? 0 : $currency->convertPrice($shippingCostUSD, $userCurrency);
    
    $taxAmount = $subtotal * $taxRate;
    $total = $subtotal + $taxAmount + $shippingCost;
    
    // Convert to minor units (cents for USD/EUR, but RWF doesn't use decimals)
    // For most currencies, multiply by 100. For zero-decimal currencies like RWF, use as-is
    $isZeroDecimalCurrency = in_array($userCurrency, ['RWF', 'JPY', 'KRW', 'VND', 'CLP']);
    ...
    foreach ($cartItems as $item) {
        $priceUSD = $item['price'] ?? 0;
        $priceInUserCurrency = $currency->convertPrice($priceUSD, $userCurrency);
        $unitAmountMinor = (int)round($priceInUserCurrency * $multiplier);
