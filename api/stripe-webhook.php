...
try {
    ...
    switch ($eventType) {
        ...
        case 'charge.succeeded':
            $processed = handle_charge_succeeded($event);
            break;
            
        case 'charge.failed':
            $processed = handle_charge_failed($event);
            break;
...
function handle_payment_intent_succeeded(array $event): bool {
    ...
    if ($type === 'wallet_topup') {
        return handle_wallet_topup_succeeded($paymentIntent, $metadata);
    }
 ...
 * Redundant with payment_intent.succeeded but provides additional confirmation
 */
function handle_charge_succeeded(array $event): bool {
    ...
    if (!$chargeId) {
        logStripe("No charge_id in event", 'error');
        return false;
    }
    
    logStripe("Processing charge.succeeded: {$chargeId}, PI: {$paymentIntentId}, amount: {$amount}, status: {$status}");
    
    // If no payment intent, this might be a direct charge (not using Payment Intents)
    if (!$paymentIntentId) {
        logStripe("Charge {$chargeId} has no payment_intent, skipping", 'warning');
        return true; // Not an error, just not using PIs
    }
    
    // Get order by payment intent
    $order = get_order_by_payment_intent($paymentIntentId);
    if (!$order) {
        logStripe("Order not found for PI {$paymentIntentId}", 'warning');
        return true; // Might be handled by payment_intent.succeeded already
    }
    
    // Log the charge information
    logStripe("Charge succeeded for order #{$order['id']}, charge: {$chargeId}");
    ...
    try {
        ...
        $stmt = $db->prepare("
            ...
            WHERE id = ? AND (payment_transaction_id IS NULL OR payment_transaction_id = '')
        ");
        $stmt->execute([$chargeId, $order['id']]);
        
        if ($stmt->rowCount() > 0) {
            logStripe("Updated order #{$order['id']} with charge ID: {$chargeId}");
        }
    } catch (Exception $e) {
        logStripe("Failed to update order with charge ID: " . $e->getMessage(), 'warning');
    }
 ...
 */
function handle_charge_failed(array $event): bool {
    $charge = $event['data']['object'] ?? [];
    $chargeId = $charge['id'] ?? null;
    $paymentIntentId = $charge['payment_intent'] ?? null;
    $failureCode = $charge['failure_code'] ?? 'unknown';
    $failureMessage = $charge['failure_message'] ?? 'No message provided';
    ...
    logStripe("Processing charge.failed: {$chargeId}, PI: {$paymentIntentId}, failure: {$failureCode} - {$failureMessage}");
    
    // If no payment intent, this might be a direct charge
    if (!$paymentIntentId) {
        logStripe("Charge {$chargeId} has no payment_intent, skipping", 'warning');
        return true;
    }
    ...
    if (!$order) {
        logStripe("Order not found for PI {$paymentIntentId}", 'warning');
        return true; // Might be handled by payment_intent.payment_failed already
    }
    
    // Mark order as failed with the charge failure reason
    $reason = "Charge failed: {$failureCode} - {$failureMessage}";
    return mark_order_failed($order['id'], $reason);
}
...
/**
 * Handle wallet top-up payment intent succeeded
 * Credits user's wallet balance when payment is successful
 */
function handle_wallet_topup_succeeded(array $paymentIntent, array $metadata): bool {
    $paymentIntentId = $paymentIntent['id'] ?? null;
    $amount = $paymentIntent['amount'] ?? 0;
    $userId = isset($metadata['user_id']) ? (int)$metadata['user_id'] : null;
    $reference = $metadata['reference'] ?? null;
    $amountUsd = floatval($metadata['amount_usd'] ?? 0);
    
    if (!$userId || !$reference) {
        logStripe("Missing user_id or reference in wallet top-up metadata", 'error');
        return false;
    }
    
    logStripe("Processing wallet top-up: PI={$paymentIntentId}, user={$userId}, amount=\${$amountUsd}");
    ...
    try {
        ...
        $newBalance = $walletService->credit(
            ...
            $amountUsd,
            $reference,
            "Wallet top-up via card - {$paymentIntentId}",
            [
        ...
        logStripe("Wallet credited successfully: user={$userId}, new_balance=\${$newBalance}");
        
        // Update pending transaction to success
        $db = getDbConnection();
        $stmt = $db->prepare("
            UPDATE wallet_transactions 
            SET type = 'credit', meta = ?
            WHERE reference = ? AND user_id = ? AND type = 'pending'
        ");
        $stmt->execute([
            json_encode([
                'payment_intent_id' => $paymentIntentId,
        ...
        logStripe("Wallet top-up failed: " . $e->getMessage(), 'error');
        return false;
    }
}
