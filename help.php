...
function createLiveChatModal() {
    const modalHTML = `
        <div id="liveChatModal" class="chat-modal">
            <div class="chat-modal-content">
                <div class="chat-header">
                    <h3>💬 Live Chat Support</h3>
                    <button class="chat-close" onclick="closeLiveChat()">&times;</button>
                </div>
                <div class="chat-body">
...
async function initializeLiveChat() {
    ...
    if (!isLoggedIn) {
        ...
        if (!name || !email) {
            alert('Name and email are required for live chat.');
            closeLiveChat();
            return;
        }
        
        // Start chat with guest info
        await startChatSession(name, email);
    } else {
...
async function startChatSession(name = '', email = '') {
    try {
        ...
        const response = await fetch('/api/live-chat.php?action=start', {
            method: 'POST',
            body: formData
        });
...
async function loadChatMessages() {
    ...
    try {
        const response = await fetch(`/api/live-chat.php?action=messages&chat_id=${chatId}&since=${lastMessageId}`);
        const data = await response.json();
...
async function sendChatMessage() {
    ...
    try {
        ...
        const response = await fetch('/api/live-chat.php?action=send', {
            method: 'POST',
            body: formData
        });
